...
from config.admin_forms import QuietAdminLoginForm
...
class OtpAdminAuthenticationForm(QuietAdminLoginForm):
    ...
    Built on `QuietAdminLoginForm` rather than Django's `AdminAuthenticationForm`
    for two reasons that both cost real time to learn. Unfold styles the login
    inputs by writing a Tailwind class list onto each widget in its own
    `__init__`, so a form outside that chain renders unstyled — white boxes on a
    white page. And the quiet form's autofill suppression has to apply to this
    login too, because there is only one: two modules each installing their own
    `admin.site.login_form` means whichever runs last silently wins, which is how
    the second factor spent this deploy switched off without a failing test.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Unfold's own __init__ only knows the two fields Django's form declares,
        # so this third one would render with no class list at all — an invisible
        # white box on a white page, the same failure the quiet form documents.
