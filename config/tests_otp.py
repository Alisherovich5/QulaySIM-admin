...
class EnrolmentCommandTests(TestCase):
    """Enrolling a device, and the ordering that keeps it from locking anyone out.

    The first version of this command created the device already confirmed. From
    that instant the login form demanded a code — whether or not the operator had
    scanned the QR, whether or not the terminal had even shown it. It locked the
    owner out of his own panel and cost the whole feature: 2FA was removed.

    So the property under test is not "a device is created". It is that a device
    only ever becomes live once somebody has proved they can produce its codes,
    and that every other path out of this command leaves the account exactly as
    it was.
    """
    ...
    def _run(self, *args, answers=None):
        """Run the command, answering its confirmation prompt with `answers`.

        `answers` is a callable, so a test can read the pending device and return
        the code its app would be showing — the secret does not exist until the
        command has created it.
        """
        ...
        out = StringIO()
        if answers is None:
            answers = self._the_right_code
        with patch("builtins.input", answers):
            call_command("setup_totp", *args, stdout=out)
        return out.getvalue()

    def _the_right_code(self, prompt=""):
        """What an operator with the app already set up would type."""
        return current_token(TOTPDevice.objects.filter(user=self.user).latest("id"))

    def test_a_confirmed_code_enrols_the_device_and_prints_its_key(self):
        output = self._run("cmd")
        device = TOTPDevice.objects.get(user=self.user)
        self.assertTrue(device.confirmed)
        self.assertIn(device.key, output)

    def test_a_wrong_code_enrols_nothing_at_all(self):
        # The regression guard. Anything left behind here — even unconfirmed —
        # is a row that a future `confirmed=True` could switch on behind the
        # operator's back, which is exactly how the lockout happened.
        output = self._run("cmd", answers=lambda prompt="": "000000")
        self.assertFalse(TOTPDevice.objects.filter(user=self.user).exists())
        self.assertIn("Nothing was enrolled", output)

    def test_a_terminal_that_cannot_be_read_enrols_nothing(self):
        def no_tty(prompt=""):
            raise EOFError

        self._run("cmd", answers=no_tty)
        self.assertFalse(TOTPDevice.objects.filter(user=self.user).exists())

    def test_an_abandoned_prompt_enrols_nothing(self):
        def ctrl_c(prompt=""):
            raise KeyboardInterrupt

        self._run("cmd", answers=ctrl_c)
        self.assertFalse(TOTPDevice.objects.filter(user=self.user).exists())

    # Throttling off: django-otp makes a device sit out a second after a failed
    # code, which is right in production and would only make this test's second
    # attempt fail for a reason it is not about.
    @override_settings(OTP_TOTP_THROTTLE_FACTOR=0)
    def test_a_mistyped_code_can_be_retried(self):
        seen = []

        def typed(prompt=""):
            seen.append(prompt)
            return "12345" if len(seen) == 1 else self._the_right_code()

        self._run("cmd", answers=typed)
        # The first answer was wrong, the second was not: the device is live.
    ...
    def test_a_failed_replacement_does_not_strand_the_account(self):
        # --replace deletes first. If the new code is never confirmed, the old
        # device is gone and the new one must be too — otherwise the operator is
        # left holding a secret nobody can produce codes for.
        self._run("cmd")
        self._run("cmd", "--replace", answers=lambda prompt="": "000000")
        self.assertFalse(TOTPDevice.objects.filter(user=self.user).exists())
